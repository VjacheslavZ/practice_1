...
class Table {
  ...
  getMaxDensity() {
    ...
    this.maxDensity = Math.max(
      ...this.data.map(row => parseFloat(row[densityColumnIndex])),
    );
    return this;
  }
  ...
  sortByInterest() {
    this.data.sort(
      (a, b) =>
        Number(b[this.config.interestColumnIndex]) -
        Number(a[this.config.interestColumnIndex]),
    );
    return this;
  }
